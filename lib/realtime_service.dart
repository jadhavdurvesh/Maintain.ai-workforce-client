...
class WorkforceRealtime {
  ...
  final StreamController<LiveTelemetry> _controller =
      StreamController<LiveTelemetry>.broadcast();
  ...
  Future<void> _connect() async {
    ...
    try {
      ...
      final response = await http
          .post(
            Uri.parse('$apiBaseUrl/api/auth/realtime-token'),
            headers: headers,
            body: '{}',
          )
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        throw Exception('Realtime authorization failed');
      }

      final data = jsonDecode(response.body) as Map<String, dynamic>;
      final realtimeToken = data['access_token']?.toString();
      if (realtimeToken == null || realtimeToken.isEmpty) {
        throw Exception('Realtime token missing');
      }

      final machinesResponse = await http
          .get(
            Uri.parse('$apiBaseUrl/api/machines'),
            headers: headers,
          )
          .timeout(const Duration(seconds: 10));

      if (machinesResponse.statusCode < 200 ||
          machinesResponse.statusCode >= 300) {
        throw Exception('Unable to resolve assigned machines');
      }
      ...
      final wsUri = Uri.parse(
        ...
        Uri.encodeQueryComponent(supabasePublishableKey) +
        '&vsn=1.0.0',
      );
      ...
      for (var index = 0; index < machineIds.length; index++) {
        ...
        final topic = 'realtime:machine:$machineId:telemetry';
        ...
        channel.sink.add(jsonEncode(<dynamic>[
          ...
          <String, dynamic>{
            'config': <String, dynamic>{
              'broadcast': <String, dynamic>{'self': false},
              'private': true,
            },
